function [ A ] =zhengjinxuanjin200( n,As,h,M,cs,C,rg )
%本子函数用于正筋选筋
%本子函数即根据裂缝选筋
%n为挡墙层数
...
if  n==1                                   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%1层挡墙
    ...
    if AsmaxAB>2454 || AsmaxAB<=0   %超出选筋库时或之前配筋计算超筋时令其配筋为0
        disp('选筋超出选筋库');
        A=[0 1000 0 1000 0 0]; 
        return      
    end
    
    AssAB=zhengjinshuchu200(AsmaxAB);         %调用函数，输出AB点的配筋，[直径 间距 实选面积]
              ...
              if w(1)<=0.2 
                 A(1,:)=[Asss(1,1) Asss(1,2) Asss(1,3) Asss(1,4) Asss(1,5) w(1)];%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%A配筋输出格式统一为[直径 间距 直径 间距 实选面积]
                  ...
                  for j=1:2000,                                                %嵌套循环增加配筋面积，最多增加2000mm2 
                       AA=zhengjinshuchu200(Asss(1,5));                %%%%%%%%%%%%%%再次选筋
...
elseif  n==2                               %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%2层挡墙
    ...
    AsmaxBC=As(1,4);                         %BC跨中计算配筋 
        ...
        if max(AsmaxAB,AsmaxBC)>2454 || min(AsmaxAB,AsmaxBC)<=0      %超出选筋库时或配筋计算超筋时令其配筋为0
           disp('选筋超出选筋库');
           A=[0 1000 0 1000 0 0;
              0 1000 0 1000 0 0]; 
           return      
    ...
    AssAB=zhengjinshuchu200(AsmaxAB);         %调用函数，输出AB点的配筋，[直径 间距 实选面积]
    AssBC=zhengjinshuchu200(AsmaxBC);         %调用函数，输出BC点的配筋，[直径 间距 实选面积]
...
for i=1:2
              ...
              if w(1)<=0.2 
                 ...
                 if Asss(i,5)>2454       %超出选筋库时令其配筋为0
                  ...
                  for j=1:2000,                                                %嵌套循环增加配筋面积，最多增加2000mm2 
                       AA=zhengjinshuchu200(Asss(i,5));                %%%%%%%%%%%%%%再次选筋
...
elseif  n==3                                   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%3层挡墙
    ...
    AsmaxCD=As(1,6);                         %CD跨中计算配筋  
        ...
        if max([AsmaxAB AsmaxBC AsmaxCD])>2454 || min([AsmaxAB AsmaxBC AsmaxCD])<=0      %超出选筋库时或配筋计算超筋时令其配筋为0
           disp('选筋超出选筋库');
           A=[0 1000 0 1000 0 0;
              0 1000 0 1000 0 0;
              0 1000 0 1000 0 0]; 
    ...
    AssAB=zhengjinshuchu200(AsmaxAB);         %调用函数，输出AB点的配筋，[直径 间距 实选面积]
    AssBC=zhengjinshuchu200(AsmaxBC);         %调用函数，输出BC点的配筋，[直径 间距 实选面积]
    AssCD=zhengjinshuchu200(AsmaxCD);         %调用函数，输出CD点的配筋，[直径 间距 实选面积]    
    Asss=[AssAB(1)  AssAB(2)  0  0  AssAB(3);
...
elseif  n==4                                   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%4层挡墙
    ...
    AsmaxDE=As(1,8);                         %DE跨中计算配筋  
        ...
        if max([AsmaxAB AsmaxBC AsmaxCD AsmaxDE])>2454 || min([AsmaxAB AsmaxBC AsmaxCD AsmaxDE])<=0     %超出选筋库时或配筋计算超筋时令其配筋为0
           disp('选筋超出选筋库');
           A=[0 1000 0 1000 0 0;
              0 1000 0 1000 0 0;
              0 1000 0 1000 0 0;
    ...
    AssBC=zhengjinshuchu200(AsmaxBC);         %调用函数，输出BC点的配筋，[直径 间距 实选面积]
    AssCD=zhengjinshuchu200(AsmaxCD);         %调用函数，输出CD点的配筋，[直径 间距 实选面积]
    AssDE=zhengjinshuchu200(AsmaxDE);         %调用函数，输出CD点的配筋，[直径 间距 实选面积]    
    Asss=[AssAB(1)  AssAB(2)  0  0  AssAB(3);
          AssBC(1)  AssBC(2)  0  0  AssBC(3);
