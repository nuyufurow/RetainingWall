function [ target ] = shuchuqianghou200( n,C,H,r,hh,p0,rg,rq,F,Qh,Qg,Z,s,cs,T )
%本子函数为收尾函数，用于输出最优墙厚（顶筋间距200），并用最优墙厚再进行内力、配筋、选筋计算输出到计算书
...
hhhh=[250 300 350 400];
  for i=1:B
      if min([AAAA(i,1) AAAA(i,2) AAAA(i,3)])==0    %把AAAA中的有0的行找出来，并赋大值与该行方便后续找最经济墙厚
         AAAA(i,:)=[999999 999999 999999 999999 999999];
      end
  end
  ss=AAAA(:,5);
  sss=find(ss==min(ss));%%%%%返回最优成本行号
  if max([AAAA(sss,1) AAAA(sss,2) AAAA(sss,3)])==999999 %若所有解配筋均存在0值，则表明内力超出程序上限或裂缝选筋超出程序上限，无法配筋计算和选筋
...
d=[Ass(3,1) fix(1000/Ass(3,2)) Ass(3,3) fix(1000/Ass(3,4))];  %重新根据实配筋计算A点裂缝，用于填计算书
wA=liefeng2(M(1),cs,d,Ass(3,5),h,C,rg); 
d=[Ass(5,1) fix(1000/Ass(5,2)) 0 0];                          %重新根据实配筋计算AB裂缝，用于填计算书
wAB=liefeng2(M(2),cs,d,Ass(5,5),h,C,rg);

%%%%%%%%%%%%%%%%%%%%%%%%%%22x6的矩阵，用于填写计算书，各行格式同计算书
target=[E 0 0 0 0 0;                                              %砼弹性模量，MPa
        ...
        As(2,3) Ass(4,1) Ass(4,2) 0        0        Ass(4,5);     %B点计算面积(mm2)、实配直径(mm)、实配间距（mm）、实配面积（mm2）
        As(2,1) Ass(3,1) Ass(3,2) Ass(3,3) Ass(3,4) Ass(3,5);     %A点计算面积(mm2)、实配通长直径(mm)、实配通长间距（mm）、实配附加直径(mm)、实配附加间距（mm）、实配面积（mm2)
        ...
        Money(2) Money(3) Money(4) Money(5) Money(7) 0];          %顶筋间距200时，顶筋费用、底筋费、砼费用、水平筋费用、总费用
...
d=[Ass(4,1) fix(1000/Ass(4,2)) Ass(4,3) fix(1000/Ass(4,4))];  %重新根据实配筋计算A点裂缝，用于填计算书
wA=liefeng2(M(1),cs,d,Ass(4,5),h(1),C,rg); 
d=[Ass(7,1) fix(1000/Ass(7,2)) 0 0];                          %重新根据实配筋计算AB裂缝，用于填计算书
wAB=liefeng2(M(2),cs,d,Ass(7,5),h(1),C,rg);  
d=[Ass(5,1) fix(1000/Ass(5,2)) Ass(5,3) fix(1000/Ass(5,4))];  %重新根据实配筋计算B点左侧裂缝，用于填计算书
wBl=liefeng2(M(3),cs,d,Ass(5,5),h(1),C,rg); 
d=[Ass(5,1) fix(1000/Ass(5,2)) Ass(5,3) fix(1000/Ass(5,4))];  %重新根据实配筋计算B点右侧裂缝，用于填计算书
wBr=liefeng2(M(3),cs,d,Ass(5,5),h(2),C,rg); 
...
target=[E 0 0 0 0 0;                                              %砼弹性模量，MPa
        ...
        As(1,1) Ass(4,1) Ass(4,2) Ass(4,3) Ass(4,4) Ass(4,5);     %A点计算面积(mm2)、实配通长直径(mm)、实配通长间距（mm）、实配附加直径(mm)、实配附加间距（mm）、实配面积（mm2)
        As(1,3) Ass(5,1) Ass(5,2) Ass(5,3) Ass(5,4) Ass(5,5);     %B点计算面积(mm2)、实配通长直径(mm)、实配通长间距（mm）、实配附加直径(mm)、实配附加间距（mm）、实配面积（mm2)
        As(1,5) Ass(6,1) Ass(6,2) 0        0        Ass(6,5);     %C点计算面积(mm2)、实配通长直径(mm)、实配通长间距（mm）、实配面积（mm2)
...
d=[Ass(5,1) fix(1000/Ass(5,2)) Ass(5,3) fix(1000/Ass(5,4))];  %重新根据实配筋计算A点裂缝，用于填计算书
wA=liefeng2(M(1),cs,d,Ass(5,5),h(1),C,rg); 
d=[Ass(9,1) fix(1000/Ass(9,2)) 0 0];                          %重新根据实配筋计算AB裂缝，用于填计算书
wAB=liefeng2(M(2),cs,d,Ass(9,5),h(1),C,rg);  
d=[Ass(6,1) fix(1000/Ass(6,2)) Ass(6,3) fix(1000/Ass(6,4))];  %重新根据实配筋计算B点左侧裂缝，用于填计算书
wBl=liefeng2(M(3),cs,d,Ass(6,5),h(1),C,rg); 
d=[Ass(6,1) fix(1000/Ass(6,2)) Ass(6,3) fix(1000/Ass(6,4))];  %重新根据实配筋计算B点右侧裂缝，用于填计算书
wBr=liefeng2(M(3),cs,d,Ass(6,5),h(2),C,rg); 
d=[Ass(10,1) fix(1000/Ass(10,2)) 0 0];                        %重新根据实配筋计算BC裂缝，用于填计算书
wBC=liefeng2(M(4),cs,d,Ass(10,5),h(2),C,rg);
d=[Ass(7,1) fix(1000/Ass(7,2)) Ass(7,3) fix(1000/Ass(7,4))];  %重新根据实配筋计算C点左侧裂缝，用于填计算书
wCl=liefeng2(M(5),cs,d,Ass(7,5),h(2),C,rg);   
d=[Ass(7,1) fix(1000/Ass(7,2)) Ass(7,3) fix(1000/Ass(7,4))];  %重新根据实配筋计算C点右侧裂缝，用于填计算书
wCr=liefeng2(M(5),cs,d,Ass(7,5),h(3),C,rg);   
...
target=[E 0 0 0 0 0;                                              %砼弹性模量，MPa
        ...
        As(3,2) As(2,2) As(4,2) 0 0 0;                            %AB跨中的受压区高度x(mm)、计算纵筋（mm2）、相对受压区高度
        As(3,1) As(2,1) As(4,1) 0 0 0;                            %A点的受压区高度x(mm)、计算纵筋（mm2）、相对受压区高度   
        As(1,1) Ass(5,1) Ass(5,2) Ass(5,3) Ass(5,4) Ass(5,5);     %A点计算面积(mm2)、实配通长直径(mm)、实配通长间距（mm）、实配附加直径(mm)、实配附加间距（mm）、实配面积（mm2)
        As(1,3) Ass(6,1) Ass(6,2) Ass(6,3) Ass(6,4) Ass(6,5);     %B点计算面积(mm2)、实配通长直径(mm)、实配通长间距（mm）、实配附加直径(mm)、实配附加间距（mm）、实配面积（mm2)
        As(1,5) Ass(7,1) Ass(7,2) Ass(7,3) Ass(7,4) Ass(7,5);     %C点计算面积(mm2)、实配通长直径(mm)、实配通长间距（mm）、实配附加直径(mm)、实配附加间距（mm）、实配面积（mm2)
        As(1,7) Ass(8,1) Ass(8,2) 0        0        Ass(8,5);     %D点计算面积(mm2)、实配通长直径(mm)、实配通长间距（mm）、实配面积（mm2)    
...
d=[Ass(6,1) fix(1000/Ass(6,2)) Ass(6,3) fix(1000/Ass(6,4))];  %重新根据实配筋计算A点裂缝，用于填计算书
wA=liefeng2(M(1),cs,d,Ass(6,5),h(1),C,rg); 
d=[Ass(11,1) fix(1000/Ass(11,2)) 0 0];                        %重新根据实配筋计算AB裂缝，用于填计算书
wAB=liefeng2(M(2),cs,d,Ass(11,5),h(1),C,rg);  
d=[Ass(7,1) fix(1000/Ass(7,2)) Ass(7,3) fix(1000/Ass(7,4))];  %重新根据实配筋计算B点左侧裂缝，用于填计算书
wBl=liefeng2(M(3),cs,d,Ass(7,5),h(1),C,rg); 
d=[Ass(7,1) fix(1000/Ass(7,2)) Ass(7,3) fix(1000/Ass(7,4))];  %重新根据实配筋计算B点右侧裂缝，用于填计算书
wBr=liefeng2(M(3),cs,d,Ass(7,5),h(2),C,rg); 
d=[Ass(12,1) fix(1000/Ass(12,2)) 0 0];                        %重新根据实配筋计算BC裂缝，用于填计算书
wBC=liefeng2(M(4),cs,d,Ass(12,5),h(2),C,rg);
d=[Ass(8,1) fix(1000/Ass(8,2)) Ass(8,3) fix(1000/Ass(8,4))];  %重新根据实配筋计算C点左侧裂缝，用于填计算书
wCl=liefeng2(M(5),cs,d,Ass(8,5),h(2),C,rg);   
d=[Ass(8,1) fix(1000/Ass(8,2)) Ass(8,3) fix(1000/Ass(8,4))];  %重新根据实配筋计算C点右侧裂缝，用于填计算书
wCr=liefeng2(M(5),cs,d,Ass(8,5),h(3),C,rg);   
d=[Ass(13,1) fix(1000/Ass(13,2)) 0 0];                        %重新根据实配筋计算CD裂缝，用于填计算书
wCD=liefeng2(M(6),cs,d,Ass(13,5),h(3),C,rg);   
d=[Ass(9,1) fix(1000/Ass(9,2)) Ass(9,3) fix(1000/Ass(9,4))];  %重新根据实配筋计算D点左侧裂缝，用于填计算书
wDl=liefeng2(M(7),cs,d,Ass(9,5),h(3),C,rg);   
d=[Ass(9,1) fix(1000/Ass(9,2)) Ass(9,3) fix(1000/Ass(9,4))];  %重新根据实配筋计算D点右侧裂缝，用于填计算书
wDr=liefeng2(M(7),cs,d,Ass(9,5),h(4),C,rg); 
...
target=[E 0 0 0 0 0 0;                                              %砼弹性模量，MPa
        ...
        As(1,1) Ass(6,1) Ass(6,2) Ass(6,3) Ass(6,4) Ass(6,5) 0;     %A点计算面积(mm2)、实配通长直径(mm)、实通长加间距（mm）、实配附加直径(mm)、实配附加间距（mm）、实配面积（mm2)
        As(1,3) Ass(7,1) Ass(7,2) Ass(7,3) Ass(7,4) Ass(7,5) 0;     %B点计算面积(mm2)、实配通长直径(mm)、实通长加间距（mm）、实配附加直径(mm)、实配附加间距（mm）、实配面积（mm2)
        As(1,5) Ass(8,1) Ass(8,2) Ass(8,3) Ass(8,4) Ass(8,5) 0;     %C点计算面积(mm2)、实配通长直径(mm)、实通长加间距（mm）、实配附加直径(mm)、实配附加间距（mm）、实配面积（mm2)
        As(1,7) Ass(9,1) Ass(9,2) Ass(9,3) Ass(9,4) Ass(9,5) 0;     %D点计算面积(mm2)、实配通长直径(mm)、实通长加间距（mm）、实配附加直径(mm)、实配附加间距（mm）、实配面积（mm2)
        As(1,9) Ass(10,1) Ass(10,2) 0      0       Ass(10,5) 0;     %E点计算面积(mm2)、实配通长直径(mm)、实配通长间距（mm）、实配面积（mm2)
        ...
        Money(2) Money(3) Money(4) Money(5) Money(7) 0 0];          %顶筋间距200时，顶筋费用、底筋费、砼费用、水平筋费用、总费用 
