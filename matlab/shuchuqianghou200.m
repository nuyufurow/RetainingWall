...
hhhh=[250; 300; 350; 400];
  for i=1:B
      if min([AAAA(i,2) AAAA(i,4) AAAA(i,6)])==0    %把AAAA中的有0的行找出来，并赋大值与该行方便后续找最经济墙厚
         AAAA(i,:)=[999999 999999 999999 999999 999999 999999];
      end
  end
  ss=AAAA(:,6);
  sss=find(ss==min(ss));%%%%%返回最优成本行号
  if max([AAAA(sss,2) AAAA(sss,4) AAAA(sss,6)])==999999 %若所有解配筋均存在0值，则表明内力超出程序上限或裂缝选筋超出程序上限，无法配筋计算和选筋
...
target=[E 0 0 0 0 0;                                              %砼弹性模量，MPa
        ...
        Money(2) Money(4) Money(5) Money(6) Money(8) 0];          %顶筋间距200时，顶筋费用、底筋费、砼费用、水平筋费用、总费用
...
target=[E 0 0 0 0 0 0;                                              %砼弹性模量，MPa
        ...
        Money(2) Money(4) Money(5) Money(6) Money(8) 0 0];          %顶筋间距200时，顶筋费用、底筋费、砼费用、水平筋费用、总费用 
