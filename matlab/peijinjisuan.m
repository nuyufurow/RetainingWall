function [ As ] = peijinjisuan( M,cs,n,h,fy,fc,ft)
...
Es=200000;
d=16;                                                                      %假设钢筋直径为16mm
kxib=beta1/(1+fy/(Es*epsiloncu));                                          %界限相对受压区高度
roumin=max(0.002,45*ft/fy/100);                                            %单侧最小配筋率，挡墙配筋需同时满足受弯构件和剪力墙的构造要求
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%1层挡墙
if n==1                                                                    %挡墙为一层时
   h0=h-d/2-cs;                                                            %截面有效高度 
   xA1=h0-sqrt(h0^2-2*abs(M(1))/(alpha1*fc*1000));                 %求A点受压区高度mm
   ...
   As1min=1000*h*roumin;
   As1=max(As11,As1min);  
   
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   xAB1=h0-sqrt(h0^2-2*abs(M(2))/(alpha1*fc*1000));                %求AB点受压区高度mm
   ...
   As2min=1000*h*roumin;
   As2=max(As22,As2min);  
   
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
   xB1=h0-sqrt(h0^2-2*abs(M(3))/(alpha1*fc*1000));                 %求B点受压区高度mm
   ...
   As33=alpha1*fc*1000*xB0/fy;                                             %B点计算配筋值 
   As3min=1000*h*roumin;
   As3=max(As33,As3min);  
   
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
As=[As1 As2 As3;       %用于后续调用
    As1 As2 As3;       %后三行用于填写计算书
    xA1 xAB1 0;
    xA1/h0 xAB1/h0 kxib];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
As=[As1 As2 max(As3l,As3r) As4 As5 0 ; %用于后续调用
    As1 As2 As3l As3r As4 As5;      %用于填写计算书
    xA1 xAB1 xB1l xB1r xBC1 0;
    xA1/h0(1) xAB1/h0(1) xB1l/h0(1) xB1r/h0(2) xBC1/h0(2) kxib];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
As=[As1 As2 max(As3l,As3r) As4 max(As5l,As5r) As6 As7 0 0;   %用于后面调用
    As1 As2 As3l As3r As4 As5l As5r As6 As7;                 %用于填写计算书
    xA1 xAB1 xB1l xB1r xBC1 xC1l xC1r xCD1 0;
    xA1/h0(1) xAB1/h0(1) xB1l/h0(1) xB1r/h0(2) xBC1/h0(2) xC1l/h0(2) xC1r/h0(3) xCD1/h0(3) kxib];

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%4层挡墙
elseif n==4
   h0=[h(1)-d/2-cs h(2)-d/2-cs h(3)-d/2-cs h(4)-d/2-cs];                               %各层截面有效高度
   xA1=h0(1)-sqrt(h0(1)^2-2*abs(M(1))/(alpha1*fc*1000));           %求A点受压区高度mm
   if xA1>=0 && xA1<=kxib*h0(1)
       xA0=xA1;
   else
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
As=[As1 As2 max(As3l,As3r) As4 max(As5l,As5r) As6 max(As7l,As7r) As8 As9 0 0 0;
    As1 As2 As3l As3r As4 As5l As5r As6 As7l As7r As8 As9;
    xA1 xAB1 xB1l xB1r xBC1 xC1l xC1r xCD1 xD1l xD1r xDE1 0;
    xA1/h0(1) xAB1/h0(1) xB1l/h0(1) xB1r/h0(2) xBC1/h0(2) xC1l/h0(2) xC1r/h0(3) xCD1/h0(3) xD1l/h0(3) xD1r/h0(4) xDE1/h0(4) kxib];   
