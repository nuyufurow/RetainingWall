...
namespace Dr.RetainingWall
{
    class Input
    {
        ...
        private double[] E;//C30弹性模量+++
        private double fc;//C30抗压强度设计值+++
        private double fy;//钢筋抗拉强度设计值
        private double r = 20 * 10 ^ -6;//填土容重N/mm3



        private double hh = 1500;//墙顶覆土厚度1500mm
        private int n=1;//层数1
        private double[] H = { 4000 };//层高4米
        private double[] h = { 300 };//单层墙厚300mm
        private double[] A;//截面积+++
        private double[] I;//截面惯性矩+++
        private Matrix K;//得到总刚度矩阵+++


        private double p0 = 5 * 10 ^ -3;//活荷载N/mm2
        private double rg = 1.3;//恒荷载分项系数
        private double rq = 1.5;//活荷载分项系数
        ...
        {
            Init();
            E[0] = dicConcrteProperty[C][1];
            fc = dicConcrteProperty[C][0];
            fy = dicRebarProperty[F][0];


            A[0] = 1000 * h[0];
            I[0] = 1000 * Math.Pow(h[0], 3) / 12;
            K = StiffnessMatrix.zonggangjuzhen(n, E, A, I, H);
        }

        private void Init()
        {
            dicConcrteProperty.Add(20, new double[] {  9.6, 2.55 * Math.Pow(10, 4), 1.54, 1.10 });
            dicConcrteProperty.Add(25, new double[] { 11.9, 2.80 * Math.Pow(10, 4), 1.78, 1.27 });
            dicConcrteProperty.Add(30, new double[] { 14.3, 3.00 * Math.Pow(10, 4), 2.01, 1.43 });
            dicConcrteProperty.Add(35, new double[] { 16.7, 3.15 * Math.Pow(10, 4), 2.20, 1.57 });
            dicConcrteProperty.Add(40, new double[] { 19.1, 3.25 * Math.Pow(10, 4), 2.39, 1.71 });
            dicConcrteProperty.Add(45, new double[] { 21.1, 3.35 * Math.Pow(10, 4), 2.51, 1.80 });
            dicConcrteProperty.Add(50, new double[] { 23.1, 3.45 * Math.Pow(10, 4), 2.64, 1.89 });
